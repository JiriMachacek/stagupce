...
//zaибtek metody __construct
   public function __construct()  //metoda kterб definuje atributy
   {
   }
...
//zaибtek metody rozeberSeo, kterб oddмlн jednotlivй informace o modulu, funkci a nбzvu jednotlivэch zбznamщ
   public function rozeberSeo($hodnoty)
   {
      ...
      $hodnoty = explode("-",$adresa,4);       //urиenн modulu a metody a pшipravenн zbytku pro rozdмlenн - rozdмlн na 3 ибsti - modul, metoda a zbytek
      $seo["modul"] = $hodnoty[0];             //uloћн do pole hodnotu modulu
      $seo["metoda"] = $hodnoty[1];            //uloћн do pole hodnotu metody
      $seo["nazev"] = $hodnoty[2];             //uloћн do pole hodnotu nбzvu
      $parametry = explode("-",$hodnoty[3]);   //rozdмlн zbytek hodnot
      $m = count($parametry);                  //poиet hodnot pшenesenэch
      $i = 0;                                  //nastavenн iteraиnн prom. pro cyklus while

      //cyklus nбm bere po dvou zбznamy, jeden pшiшadн jako identifikбtor v poli a druhэ jako hodnotu
      while($i<=$m)//pojede tak dlouho dokud i nebude stejnм velkй nebo menљн jako poиet rozdмlenэch hodnot
        {
        $klic=$parametry[$i];          //prvnн z dvojce hodnot je klни pole
        $i++;                          //prvnн inkrementace, pшeneseme se na druhou hodnotu
        $seo[$klic]=$parametry[$i];    //druhэ je pak hodnota
        $i++;                          //druhб inkrementace, inkremetuje podruhй i, abysme v dalљнm obмhu cyklu nepracovali s tнmto i
        }
      //------------------------
...
//zaибtek metody slozSeo, kterб sloћн jednotlivй informace o modulu, funkci a nбzvu jednotlivэch zбznamщ
   public function slozSeo($modul,$metoda,$nazev,$poleHodnot)
   {
      //zavolбnн metody kterб nбm upravн nбzev pro seo
      $nazev = upravProSeo($nazev);
      //------------------------

      $m = count($poleHodnot);    //poиet hodnot pшenesenэch
      $i = 0;                     //nastavenн iteraиnн prom. pro cyklus while
      $hodnoty = "";              //vynulovбnн prom. pro pшidбvбnн hodnot

      //cyklus nбm bere po dvou zбznamy, jeden pшiшadн jako identifikбtor v poli a druhэ jako hodnotu
      while($i<=$m)//pojede tak dlouho dokud i nebude stejnм velkй nebo menљн jako poиet rozdмlenэch hodnot
        {
        $hodnoty=$hodnoty."-";               //dodбnн dмlнcнho znaku
        $hodnoty=$hodnoty.$poleHodnot[$i];   //prvnн z dvojce hodnot je klни pole
        $i++;                                //prvnн inkrementace, pшeneseme se na druhou hodnotu
        $hodnoty=$hodnoty."-";               //dodбnн dмlнcнho znaku
        $hodnoty=$hodnoty.(upravProSeo($poleHodnot[$i]));   //pro jistotu hodnoty jeљtм optimalizujeme a odstranнme diakritiku
        $i++;                                //druhб inkrementace, inkremetuje podruhй i, abysme v dalљнm obмhu cyklu nepracovali s tнmto i
        }
      //------------------------

      //sloћenн hodnot
      $seo = $modul."-".$metoda."-".$nazev.$hodnoty; //sloћenн jednotlivэch ибstн, prom. hodnoty uћ v sobм dмlнcн znaky obsahuje
      //------------------------
